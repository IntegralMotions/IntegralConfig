...
bool MPackObject::readHeader(mpack_reader_t &reader, MPackHeader &header) {
  ...
  switch (header.type) {
  case mpack_type_map:
    header.countOrLength = mpack_tag_map_count(&t);
    break;
  case mpack_type_array:
    header.countOrLength = mpack_tag_array_count(&t);
    break;
  case mpack_type_str:
    header.countOrLength = mpack_tag_str_length(&t);
    break;
  case mpack_type_bin:
    header.countOrLength = mpack_tag_bin_length(&t);
    break;
  default:
    break;
  }
...
void MPackObject::read(mpack_reader_t &reader, int depth) {
  ...
  for (size_t i = 0; i < header.countOrLength; i++) {
    ...
    if (isArray(key)) {
      ...
      if (!readHeader(reader, arrayHeader) ||
          arrayHeader.type != mpack_type_array) {
        mpack_reader_flag_error(&reader, mpack_error_type);
        delete[] key;
        return;
      }
    ...
    mpack_tag_t t = mpack_peek_tag(&reader);
    mpack_type_t type = mpack_tag_type(&t);
    switch (type) {
    case mpack_type_bool: {
      bool v = mpack_expect_bool(&reader);
      setMPackValue(key, &v, type);
    } break;
    case mpack_type_int: {
    ...
    case mpack_type_str: {
      MPackHeader valueHeader{};
      if (!readHeader(reader, valueHeader)) {
        delete[] key;
        return;
      }
      char *buf = new char[valueHeader.countOrLength + 1];
      if (valueHeader.countOrLength)
        mpack_read_bytes(&reader, buf, valueHeader.countOrLength);
      buf[valueHeader.countOrLength] = '\0';
      mpack_done_str(&reader);
      setMPackValue(key, buf, type);
      delete[] buf;
    } break;
    case mpack_type_bin: {
      MPackHeader valueHeader{};
      ...
      mpack_skip_bytes(&reader, valueHeader.countOrLength);
    } break;
    default:
      mpack_reader_flag_error(&reader, mpack_error_type);
      break;
...
bool MPackObject::isObject(const char *name) { return false; }

bool MPackObject::isArray(const char *name) { return false; }

MPackObject *MPackObject::getObject(const char *name) { return nullptr; }

void *MPackObject::getArray(const char *name, size_t count, CppType &outType) {
  return nullptr;
}