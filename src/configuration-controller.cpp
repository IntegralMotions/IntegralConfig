#include "configuration-controller.h"
...
ConfigurationController::ConfigurationController(Communication *comm) {
  ...
  mpack_reader_set_fill(
      &_reader, +[](mpack_reader_t *r, char *buffer, size_t count) -> size_t {
        auto *self =
            static_cast<ConfigurationController *>(mpack_reader_context(r));
...
ConfigurationController &ConfigurationController::get() { return *_instance; }

void ConfigurationController::loop() { readMpack(); }
