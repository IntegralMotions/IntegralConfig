...
private:
  ...
  void *getArray(const char *n, size_t count, CppType &outType) override {
    ...
    if (!strcmp(n, "aa")) {
      ...
      outType = CppType::NestedArray;
      return aa;
    }
    return nullptr;
  }
  MPackObject *getObject(const char *) override { return new Elem(); }
  void *getNestedArray(const char *n, size_t idx, size_t cnt,
                       CppType &outInnerType) override {
    if (strcmp(n, "aa") == 0 && idx < aan) {
      delete[] aa[idx];
      aalen[idx] = cnt;
      aa[idx] = new int32_t[cnt]{};
      outInnerType = CppType::I32;
      return aa[idx];
    }
    return nullptr;
  }
